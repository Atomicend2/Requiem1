  ...
  try {
    ...
    for (let i = 1; i <= 5; i++) {
      try {
        ...
        console.log(`  [git] ✓ Pushed at page ${pageNum} (${totalCards} cards, cards.json only)`);
        return true;
      } catch {
        console.warn(`  [git] Push attempt ${i} failed, waiting 15s...`);
        execSync("sleep 15");
      }
    }
    console.warn(`  [git] All push attempts failed at page ${pageNum}`);
    return false;
  } catch (e) {
